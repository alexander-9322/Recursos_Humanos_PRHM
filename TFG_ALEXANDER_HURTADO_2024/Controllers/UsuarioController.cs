...
namespace DB.Models.Controllers
{
    public class UsuarioController : Controller
    {
        ...
        {
            var distritos = db.distritoes.Where(d => d.Canton_idCanton == IdCanton).ToList();
        ...
        public ActionResult Edit()
        {
            ...
            bool mostrarClaveConfirmacion = user.USUARIO.PrimerIngreso;
            // Pasar la variable a la vista
            ViewBag.MostrarClaveConfirmacion = mostrarClaveConfirmacion;
        ...
        {
            bool mostrarClaveConfirmacion = user.USUARIO.PrimerIngreso;
            if (user.USUARIO.Clave != null && user.USUARIO.Correo != null)
            {
                

                if (mostrarClaveConfirmacion == true)
                {
                    if (!user.USUARIO.Clave.Equals(user.USUARIO.ClaveConfirmacion))
                    {
                        ViewBag.MostrarClaveConfirmacion = mostrarClaveConfirmacion;
        ...
        public ActionResult RestablecerContrasenna(long cedula)
        {
            DataSet ds = new DataSet();
            Servicios se = new Servicios();
            ...
            int idUsuario = user.USUARIO.IdUsuario;

            usuario uSUARIO = db.usuarios.FirstOrDefault(u => u.IdUsuario == idUsuario);
            uSUARIO.Clave = "Test12345";
