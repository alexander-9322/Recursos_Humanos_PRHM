...
namespace TFG_ALEXANDER_HURTADO_2024.Controllers
{
    public class VacacionesController : Controller
    {
        ...
        public ActionResult Create(Vacaciones vACACIONE)
        {
            try
            {
                if (vACACIONE.VACACION.Cantidad_Dias != 0 && vACACIONE.VACACION.Fecha_Inicio != null)
                {

                    // Validar que la fecha de inicio no sea un domingo
                    if (vACACIONE.VACACION.Fecha_Inicio.DayOfWeek == DayOfWeek.Sunday)
                    {
                        Session["Error"] = "La fecha de inicio no puede ser un domingo.";
                    ...
                    if (cantidadDias > 0)
                    {
                        if (cantidadDias >= vACACIONE.VACACION.Cantidad_Dias)
                        {
                            ds = se.Crear_Vacaciones(vACACIONE);
                        }
                        else
                        {
                            Session["Error"] = "No tienes esa cantidad de vacaciones disponibles. Cantidad de dias disponibles: " + cantidadDias.ToString();
                        }
                       
                    }
                    else
                    {
                        Session["Error"] = "No tienes vacaciones disponibles. Cantidad de dias disponibles: " + cantidadDias.ToString();
                    }
        ...
        public ActionResult Edit(DateTime fechaInicio, int cantidadDias, long cedula, string estado, string comentario, int idEmpleado)
        {
            ...
            var vacacion = db.vacaciones.FirstOrDefault(v => v.Fecha_Inicio == fechaInicio && v.Cantidad_Dias == cantidadDias && v.Empleados_Id_Empleado == idEmpleado);
